...
def arm_object_grasp(config):
    ...
    with bosdyn.client.lease.LeaseKeepAlive(lease_client, must_acquire=True, return_at_exit=True):
        ...
        # 2. COLORIZE: If it's a depth image, apply a heatmap
        if image.shot.image.pixel_format == image_pb2.Image.PIXEL_FORMAT_DEPTH_U16:
            # Scale depth (typically 0-3000mm) to 8-bit (0-255)
            # Dividing by 30 means ~7.5 meters is the max range shown
            img_8bit = (img / 30).astype(np.uint8)
            img = cv2.applyColorMap(img_8bit, cv2.COLORMAP_JET)
        ...
        cv2.resizeWindow(image_title, 1920, 1080) 
...
def main():
    ...
    parser.add_argument('-p', '--palm-to-fingertip', type=float, default=0.5,
                    help='Value from 0.0 (palm) to 1.0 (tips). Default is 0.5')
